# Drafting brief — Book 18 essay 07
...
your contract is in section 6 under essay 07), `AGENT.md`, `TEMPLATE.md`, then
...
- `chapters/07-eudi-wallet-acceptance.html` — the essay. Copy the exact skeleton of chapter 01: same head, header,
  `p.kicker` ("Essay 07 of 7 · <candidate name>"), argued `h1`, `p.lede`, the sections
...
- `checks/claims/07.tsv` — header `id	claim	source_url	source_locator	status`, tab-separated,
  exactly five columns, ids `07-slug`, status `verified` / `inference` / `open`. Ship with
  zero `open`.
- `resources/sources/07/SOURCES.md` — numbered index: publisher, title, URL, accessed date,
  what it supports (claim ids).
- `resources/sources/07/excerpts/<slug>.txt` — one file per `verified` row: first line the
...
- `drafts/07.notes.md` — optional working notes.
...
Run `python3 checks/structure.py 07` from `/home/diablo/book18` until it passes with no
